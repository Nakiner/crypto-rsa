...
namespace Курсовая
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if(ip.Ip == "")
            {
                MessageBox.Show("Не удалось получить IP адрес.");
                this.Close();
            }
            if(ip.Banned(ip.Ip))
            {
                if (Convert.ToInt32(sql.Select("attpt", "bans", "ip", ip.Ip, 0)) == 3)
                {
                    long curr = Convert.ToInt64(sql.Reverse(sql.Date()));
                    long unban = Convert.ToInt64(sql.Reverse(sql.Select("date", "bans", "ip", ip.Ip, 0)));
                    if (unban > curr)
                    {
                        MessageBox.Show("Вы заблокированы в системе.\nДата разблокировки: " + sql.Select("date", "bans", "ip", ip.Ip, 0));
                        this.Close();
                    }
                    else sql.Delete("bans", "ip", ip.Ip);
                }
            }
            if(sql.username == "")
            {
